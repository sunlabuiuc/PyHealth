...
from pyhealth.interpret.methods import Ensemble
...
class _DummyInterpreter(BaseInterpreter):
    """Dummy interpreter that returns random attributions."""

    def attribute(self, **kwargs) -> dict:
        attributions = {}
        for key, value in kwargs.items():
            if isinstance(value, torch.Tensor):
                attributions[key] = torch.randn_like(value)
        return attributions
...
class TestEnsembleFlattenValues(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create 3 dummy interpreters
        self.interpreters = [
            _DummyInterpreter(self.model),
            _DummyInterpreter(self.model),
            _DummyInterpreter(self.model),
        ]

        self.ensemble = Ensemble(self.model, self.interpreters)
    ...
    def test_flatten_single_feature_1d(self):
        ...
        flattened = self.ensemble._flatten_attributions(values)
...
class TestEnsembleUnflattenValues(unittest.TestCase):
    ...
    def test_unflatten_single_feature_1d(self):
        ...
        unflattened = self.ensemble._unflatten_attributions(flattened, shapes)
...
class TestEnsembleRoundtrip(unittest.TestCase):
    ...
    def test_roundtrip_single_feature(self):
        ...
        flattened = self.ensemble._flatten_attributions(original)
        unflattened = self.ensemble._unflatten_attributions(flattened, shapes)
