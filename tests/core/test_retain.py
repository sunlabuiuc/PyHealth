...
from pyhealth.models import RETAIN, MultimodalRETAIN
...
class TestMultimodalRETAIN(unittest.TestCase):
        ...
        self.samples = [
            {
                "patient_id": "patient-0",
                "visit_id": "visit-0",
                "conditions": [["A", "B"], ["C"]],       # nested sequence (sequential)
                "procedures": [["P1"], ["P2", "P3"]],    # nested sequence (sequential)
                "demographics": ["asian", "male"],         # multi-hot (non-sequential)
                "vitals": [120.0, 80.0, 98.6],           # tensor (non-sequential)
                "label": 1,
            },
            {
                "patient_id": "patient-1",
                "visit_id": "visit-1",
                "conditions": [["D"], ["E", "F"]],        # nested sequence (sequential)
                "procedures": [["P4"]],                    # nested sequence (sequential)
                "demographics": ["white", "female"],       # multi-hot (non-sequential)
                "vitals": [110.0, 75.0, 98.2],           # tensor (non-sequential)
                "label": 0,
            },
        ]
    ...
    def test_model_forward(self):
        """Test that the MultimodalRETAIN forward pass works correctly."""
        train_loader = get_dataloader(self.dataset, batch_size=2, shuffle=True)
        data_batch = next(iter(train_loader))
    ...
    def test_model_backward(self):
        """Test that the MultimodalRETAIN backward pass works correctly."""
        train_loader = get_dataloader(self.dataset, batch_size=2, shuffle=True)
        data_batch = next(iter(train_loader))
    ...
    def test_custom_hyperparameters(self):
        """Test MultimodalRETAIN with custom hyperparameters."""
    ...
    def test_with_simple_sequences(self):
        """Test MultimodalRETAIN with simple (non-nested) sequences mixed with non-sequential."""
        samples = [
            {
                "patient_id": "patient-0",
                "visit_id": "visit-0",
