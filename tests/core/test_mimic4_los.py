...
from pyhealth.tasks.length_of_stay_prediction import LengthOfStayPredictionMIMIC4


class TestMIMIC4LengthOfStayPrediction(unittest.TestCase):
    """Test MIMIC-4 length of stay prediction task with demo data from local test resources."""
    ...
    def test_length_of_stay_prediction_mimic4_set_task(self):
        """Test LengthOfStayPredictionMIMIC4 task with set_task() method."""
        print(f"\n{'='*60}")
        print("TEST: test_length_of_stay_prediction_mimic4_set_task()")
        print(f"{'='*60}")

        print("Initializing LengthOfStayPredictionMIMIC4 task...")
        task = LengthOfStayPredictionMIMIC4()

        # Test that task is properly initialized
        print(f"✓ Task initialized: {task.task_name}")
        self.assertEqual(task.task_name, "LengthOfStayPredictionMIMIC4")
        ...
        try:
            ...
            if num_samples > 0:
                ...
                print(
                    f"\n✓ test_length_of_stay_prediction_mimic4_set_task() passed successfully"
                )
            ...
            import traceback
            traceback.print_exc()
            self.fail(f"Failed to use set_task with LengthOfStayPredictionMIMIC4: {e}")
