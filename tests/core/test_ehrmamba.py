...
class TestEHRMamba(unittest.TestCase):
    """Test cases for the EHRMamba model."""
    ...
    def setUp(self):
        ...
        self.model = EHRMamba(dataset=self.dataset, embedding_dim=128, num_layers=1)

    def test_model_initialization(self):
        """Test that the EHRMamba model initializes correctly."""
        self.assertIsInstance(self.model, EHRMamba)
    ...
    def test_model_forward(self):
        """Test that the EHRMamba forward pass works correctly."""
    ...
    def test_model_backward(self):
        """Test that the EHRMamba backward pass works correctly."""
        train_loader = get_dataloader(self.dataset, batch_size=2, shuffle=True)
        data_batch = next(iter(train_loader))
    ...
    def test_model_with_embedding(self):
        """Test that EHRMamba returns embeddings when requested."""
        train_loader = get_dataloader(self.dataset, batch_size=2, shuffle=True)
        data_batch = next(iter(train_loader))
    ...
    def test_custom_hyperparameters(self):
        """Test EHRMamba with custom hyperparameters."""
        model = EHRMamba(
        ...
        self.assertEqual(model.state_size, 8)
