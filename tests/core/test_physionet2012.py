...
@patch("pyhealth.datasets.physionet2012.urllib.request.urlretrieve")
@patch("pyhealth.datasets.physionet2012.tarfile.open")
def test_physionet2012_dataset_download_parse(mock_tarfile, mock_urlretrieve, mock_in_notebook):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        def mock_extractall(*args, **kwargs):
            ...
            with open(os.path.join(set_a_dir, "132791.txt"), "w") as f:
                f.write("Time,Parameter,Value\n"
            ...
            outcomes_content = "RecordID,In-hospital_death\n132801,0\n132791,1\n"
            with open(os.path.join(temp_dir, "Outcomes-a.txt"), "w") as f:
                f.write(outcomes_content)

        mock_tar_ctx = mock_tarfile.return_value.__enter__.return_value
        mock_tar_ctx.extractall.side_effect = mock_extractall
