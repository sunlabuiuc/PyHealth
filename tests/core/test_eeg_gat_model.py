...
class TestEEGGATConvNet(unittest.TestCase):

    def setUp(self) -> None:
        self.dataset = _make_dataset()
        self.model   = EEGGATConvNet(dataset=self.dataset)
        self.model.eval()
    ...
    def test_model_instantiates(self) -> None:
        self.assertIsInstance(self.model, EEGGATConvNet)
    ...
    def test_default_dropout_stored(self) -> None:
        self.assertEqual(self.model.dropout, 0.2)

    def test_default_attn_dropout_stored(self) -> None:
        self.assertEqual(self.model.attn_dropout, 0.0)

    def test_custom_dropout_stored(self) -> None:
        model = EEGGATConvNet(dataset=self.dataset, dropout=0.3, attn_dropout=0.6)
        self.assertEqual(model.dropout, 0.3)
        self.assertEqual(model.attn_dropout, 0.6)
    ...
    def test_forward_with_attn_dropout(self) -> None:
        model = EEGGATConvNet(dataset=self.dataset, dropout=0.2, attn_dropout=0.6)
        model.train()
        batch = self._get_batch()
        out = model(**batch)
