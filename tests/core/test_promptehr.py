...
class TestPromptEHR(unittest.TestCase):
    """Test cases for the PromptEHR synthetic-EHR generator."""
    ...
    def test_model_initialization(self):
        ...
        self.assertIsInstance(self.model, PromptEHR)
        ...
        self.assertEqual(self.model.mask_id, proc_vocab + 3)
        self.assertEqual(self.model.code_prompt_id, proc_vocab + 4)
        self.assertEqual(self.model.bart.config.vocab_size, proc_vocab + 5)
        # The learnable soft prompt is part of the module parameters.
        self.assertEqual(
            tuple(self.model.soft_prompt.shape), (4, 16)
        )
    ...
    def test_model_forward(self):
        ...
        # y_prob: (B, L_dec, vocab_size)
        self.assertEqual(ret["y_prob"].shape[0], 2)
        self.assertEqual(ret["y_prob"].shape[2], self.model.bart.config.vocab_size)

    def test_model_backward(self):
        """Backward populates gradients on model parameters (incl. soft prompt)."""
        loader = get_dataloader(self.dataset, batch_size=2, shuffle=False)
        batch = next(iter(loader))
        ...
        # The soft prompt specifically should receive a gradient.
        self.assertIsNotNone(self.model.soft_prompt.grad)
