...
class TestDREAMTDatasetNewerVersions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.root = Path(cls.temp_dir)
        (cls.root / "data_64Hz").mkdir()
        (cls.root / "data_100Hz").mkdir()
        cls.num_patients = 5
        rng = np.random.default_rng(42)
        patient_data = {
            'SID': [f"S{i:03d}" for i in range(1, cls.num_patients + 1)],
            'AGE': rng.uniform(25, 65, cls.num_patients),
        ...
        pd.DataFrame(patient_data).to_csv(cls.root / "participant_info.csv", index=False)
        for i in range(1, cls.num_patients + 1):
            sid = f"S{i:03d}"
            partial_data = {
                'TIMESTAMP': [rng.uniform(0, 100)],
                'BVP': [rng.uniform(1, 10)],
                'HR': [int(rng.integers(15, 100))],
                'EDA': [rng.uniform(0, 1)],
                'TEMP': [rng.uniform(20, 30)],
                'ACC_X': [rng.uniform(1, 50)],
                'ACC_Y': [rng.uniform(1, 50)],
                'ACC_Z': [rng.uniform(1, 50)],
                'IBI': [rng.uniform(0.6, 1.2)],
                'Sleep_Stage': [rng.choice(['W', 'N1', 'N2', 'N3', 'R'])],
            }
            pd.DataFrame(partial_data).to_csv(cls.root / "data_64Hz" / f"{sid}_whole_df.csv")
            pd.DataFrame(partial_data).to_csv(cls.root / "data_100Hz" / f"{sid}_PSG_df.csv")
        cls.dataset = DREAMTDataset(root=str(cls.root))
    ...
    def test_dataset_initialization(self):
        ...
        self.assertEqual(self.dataset.dataset_name, "dreamt_sleep")
    ...
    def test_patient_count(self):
        self.assertEqual(len(self.dataset.unique_patient_ids), self.num_patients)
