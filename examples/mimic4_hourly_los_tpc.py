"""
MIMIC-IV hourly length-of-stay (LoS) training and evaluation script for the
Temporal Pointwise Convolution (TPC) model.

This module provides an end-to-end training script for reproducing and
evaluating the Temporal Pointwise Convolution (TPC) architecture on the
MIMIC-IV hourly remaining length-of-stay prediction task. It is designed
to run within the PyHealth project structure and serves as the required
example script for model evaluation and reproducible experimentation.

Overview:
    This script performs the full experimental pipeline for hourly ICU
    remaining length-of-stay prediction on MIMIC-IV data:

    1. Load MIMIC-IV tables through PyHealth using a custom dataset config.
    2. Construct hourly LoS task samples with time-series and static features.
    3. Convert task samples into model-ready tensors.
    4. Train and validate the TPC model.
    5. Evaluate the trained model on a held-out test split.

    The script is intended both as a reproducible training entry point and
    as documentation of how the TPC model is used within the PyHealth
    framework for MIMIC-IV experiments.

Key Components:
    - set_seed: Reproducible random seeding for Python, NumPy, and PyTorch
    - build_categorical_vocab: Training-only vocabulary construction for
      categorical static features
    - SimpleLoSDataset: Wrapper that converts task samples into TPC-ready
      tensors
    - collate_fn: Variable-length batch padding with static feature width
      normalization
    - evaluate: Validation and test loop with masked sequence metrics
    - main: Full dataset loading, training, validation, and test pipeline

Inputs:
    root: Path to the MIMIC-IV dataset root directory. This may be supplied
        via ``--root`` or the ``MIMIC4_ROOT`` environment variable.
    Task samples:
        Hourly task samples generated by ``HourlyLOSEICU`` containing:
        - time-series values, masks, and decay channels
        - numeric static features
        - optional categorical static features
        - hourly remaining LoS targets

Outputs:
    - Trained TPC model in memory for the current run
    - Console logs for training, validation, and test metrics
    - Summary line reporting validation and test loss, MAE, and RMSE

Implementation Notes:
    - This script supports the example and evaluation requirements of the
      project rubric by providing a reproducible training and testing
      pipeline for MIMIC-IV.
    - Supported configuration variants include:
        * full vs no-decay channel configurations
        * categorical static inclusion/exclusion
        * MSLE vs MSE loss
        * configurable maximum ICU horizon
    - Unlike the eICU script, this version is focused on train/validation/test
      evaluation rather than the broader architectural ablation set.
    - The data pipeline is designed to work with synthetic, dev, or real
      MIMIC-IV data, depending on the provided dataset root and CLI options.
    - Optional debug prediction printing may be enabled with ``--debug_examples``.

Example:
    >>> MIMIC4_ROOT=/path/to/mimiciv/3.1 PYTHONPATH=. \\
...
for hourly ICU length-of-stay prediction and supports reproducible evaluation
...
def evaluate(
    ...
    Args:
        ...
        debug_examples: Whether to print one sample prediction/target pair.
    ...
    if debug_examples and total_count > 0:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "Path to MIMIC-IV dataset root (directory containing hosp/ and icu/). "
            "Defaults to the MIMIC4_ROOT environment variable when set."
        ),
    )
    ...
    parser.add_argument(
        "--debug_examples",
        action="store_true",
        help="Print one example prediction/target pair during evaluation.",
    )
...
def main():
    ...
    print(f"debug_examples: {args.debug_examples}")
