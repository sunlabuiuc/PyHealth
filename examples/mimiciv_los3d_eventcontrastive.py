...
"""
Ablation study for EBCLModel using a real PyHealth MIMIC-IV length-of-stay task.

Pipeline:
1. Load a MIMIC-IV dataset through PyHealth.
2. Apply the built-in LengthOfStayPredictionMIMIC4 task.
3. Preprocess the task samples into EBCL-style inputs:
   ...
   - post_mask
   - label
4. Convert the built-in multiclass LOS category into a binary LOS-3D proxy:
   - label = 1 if los >= 3
   - label = 0 otherwise
5. Perform a label-aware split so both train and validation sets contain
   both binary classes.
6. Wrap the processed samples with create_sample_dataset so EBCLModel can be
   trained without changing its input interface.
7. Compare several hyperparameter settings.

Notes:
- PyHealth's built-in MIMIC-IV LOS task outputs a 10-class LOS category
  under the key "los", not a binary LOS-3-day label.
- This script converts that multiclass output into a binary LOS-3D proxy for
  compatibility with the current EBCLModel finetuning head.
...
How to run:
    python3 examples/mimiciv_los3d_eventcontrastive.py

Before running:
- Update MIMIC4_PATH to your local MIMIC-IV path.
- Make sure your PyHealth version includes MIMIC4EHRDataset.
"""
...
def preprocess_task_samples(
    ...
    """Converts LengthOfStayPredictionMIMIC4 samples into EBCL-style samples.

    The built-in LOS task emits:
        - conditions
        - procedures
        - drugs
        - los (10-class category)

    We convert it into:
        - pre: conditions + procedures
        - post: drugs
        - label: binary LOS-3D proxy from the multiclass los category
    """
    vocab: Dict[str, int] = {}
    ...
    for i in range(len(task_dataset)):
        ...
        los_category = int(sample.get("los", 0))
        label = int(los_category >= 3)
...
def print_results_table(results: List[Dict]) -> None:
    ...
    print("\nAblation Results: EBCLModel on MIMIC-IV LOS-3D Proxy Task")
    print("-" * 90)
    print(
        f"{'Config':<18}"
        f"{'LR':<10}"
...
def main() -> None:
    """Runs the MIMIC-IV LOS-3D proxy ablation study."""
    ...
    task_dataset = base_dataset.set_task(LengthOfStayPredictionMIMIC4())
    ...
    train_dataset = build_ebcl_sample_dataset(
        train_samples,
        dataset_name="mimic4_los3d_eventcontrastive_train",
    )
    val_dataset = build_ebcl_sample_dataset(
        val_samples,
        dataset_name="mimic4_los3d_eventcontrastive_val",
    )
