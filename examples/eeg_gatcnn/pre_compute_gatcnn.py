"""Pre-compute EEG graph features from raw TUAB/LEMON data and run GAT inference.

This script demonstrates the full raw-EEG → GAT pipeline:
...
Or supply a different data root:
    ...
    python examples/eeg_gatcnn/pre_compute_gatcnn.py --root /path/to/eeg-gcnn-data

The script prints each processing stage, the shape of the produced tensors,
and performs a single forward pass through EEGGATConvNet to verify
end-to-end compatibility.

Note
----
EEGGATConvNet uses float64 internally (required by PyTorch Geometric's GAT
layer on CPU). The script casts inputs accordingly.
"""
...
# ---------------------------------------------------------------------------
# Stage 3 — Model: single forward pass through EEGGATConvNet
# ---------------------------------------------------------------------------

def run_model(sample_dataset, batch_size: int) -> None:
    print(f"\n{'='*60}")
    print("  Stage 3 — Model: EEGGATConvNet")
    print(f"{'='*60}")
    ...
    # GAT requires float64 on CPU for PyTorch Geometric's attention layers.
    model = EEGGATConvNet(dataset=dummy_ds, num_node_features=6).double()
    model.eval()
    ...
    # Cast to float64 for GAT.
    node_features_batch = torch.stack(node_features_list).double()  # (B, 8, 6)
    adj_matrix_batch = torch.stack(adj_matrix_list).double()        # (B, 8, 8)
    labels_batch = torch.DoubleTensor(labels_list).unsqueeze(1)     # (B, 1)

    print(f"\n  Input  node_features : {node_features_batch.shape}  dtype={node_features_batch.dtype}")
    print(f"  Input  adj_matrix    : {adj_matrix_batch.shape}  dtype={adj_matrix_batch.dtype}")
    ...
    print(f"\n  ✓ GAT forward pass successful — tensors flow end-to-end from")
...
def main() -> None:
    ...
    print("\nEEG-GCNN Pre-compute Pipeline  (GAT)")
    print("=" * 60)
    print(f"  Paper  : Wagh & Varatharajah (2020), ML4H @ NeurIPS 2020")
    print(f"  Model  : EEGGATConvNet (GAT)")
    ...
    print("  Outputs are ready for training — see training_pipeline_shallow_gatcnn.py")
    print("=" * 60)
