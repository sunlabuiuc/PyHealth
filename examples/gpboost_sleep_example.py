...
try:
    ...
    if "numpy.dtype size changed" in str(e):
        ...
        print(
            "2. Ensure consistent versions: pip install 'numpy==1.23.5' 'pandas==1.5.3'"
        )
        sys.exit(1)
    else:
...
def print_sample_data_overview(data):
    ...
    print(
        f"Patient {patient_id} - {total_epochs} epochs ({wake_epochs} awake, {sleep_epochs} asleep)"
    )
    print(
        f"Random effects: Obesity = {sample_patient['obesity']}, Apnea = {sample_patient['apnea']}"
    )
    ...
    for i, visit in enumerate(visits[:5]):
        print(
            f"{i:4d} | {visit['sleep_stage']:11s} | {visit['heart_rate']:9.1f} | {visit['eda']:.3f} | "
    ...
    all_acc = [
        visit["accelerometer"] for patient in data for visit in patient["visits"]
    ]

    print(
        f"Heart Rate: min={min(all_hr):.1f}, max={max(all_hr):.1f}, mean={np.mean(all_hr):.1f}, std={np.std(all_hr):.1f}"
    )
    print(
        f"EDA: min={min(all_eda):.3f}, max={max(all_eda):.3f}, mean={np.mean(all_eda):.3f}, std={np.std(all_eda):.3f}"
    )
    print(
        f"Temperature: min={min(all_temp):.1f}, max={max(all_temp):.1f}, mean={np.mean(all_temp):.1f}, std={np.std(all_temp):.1f}"
    )
    print(
        f"Movement: min={min(all_acc):.4f}, max={max(all_acc):.4f}, mean={np.mean(all_acc):.4f}, std={np.std(all_acc):.4f}"
    )

    all_stages = [
        visit["sleep_stage"] for patient in data for visit in patient["visits"]
    ]
...
def print_sample_predictions(model, test_data):
    ...
    if len(y_prob) >= n_samples:
        sample_prob = (
            y_prob[:n_samples, 0] if len(y_prob.shape) > 1 else y_prob[:n_samples]
        )
        ...
        for t in range(n_samples):
            ...
            print(
                f"{t:4d} | {true_state:10s} | {pred_state:14s} | {float(sample_prob[t]):.4f}"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Average epochs per patient: {sum(len(p['visits']) for p in data)/len(data):.1f}"
    )
