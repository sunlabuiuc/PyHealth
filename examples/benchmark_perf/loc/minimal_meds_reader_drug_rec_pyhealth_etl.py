...
def get_drug_rec_samples(subjects: Iterator[meds_reader.Subject]):
    ...
    for subject in subjects:
        ...
        if len(sorted_visits) < 2: continue
        for i, (vid, cur) in enumerate(sorted_visits):
            if not (cur['conditions'] and cur['procedures'] and cur['drugs']): continue
            samples.append({"visit_id": vid, "patient_id": subject.subject_id,
                            "conditions": [list(sorted_visits[j][1]['conditions']) for j in range(i+1)],
                            "procedures": [list(sorted_visits[j][1]['procedures']) for j in range(i+1)],
