"""Benchmark script for MIMIC-IV mortality prediction with num_workers=1.

This benchmark measures:
1. Time to load base dataset
2. Time to process task with num_workers=1
...
def main():
    ...
    print("=" * 80)
    print(f"BENCHMARK: num_workers=1, dev={dev}")
    if enable_memory_limit:
    ...
    cache_root = "./benchmark_cache/workers_1"
    if dev:
        cache_root += "_dev"
    ...
    # STEP 2: Apply StageNet mortality prediction task with num_workers=1
    print("\n[2/2] Applying mortality prediction task (num_workers=1)...")
