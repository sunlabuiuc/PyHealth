"""
eICU hourly length-of-stay (LoS) training and ablation script for the
Temporal Pointwise Convolution (TPC) model.

This module provides an end-to-end training script for reproducing and
ablation-testing the Temporal Pointwise Convolution (TPC) architecture on
the eICU hourly remaining length-of-stay prediction task. It is designed
...
Overview:
    ...
    remaining length-of-stay prediction on eICU data:

    1. Load eICU tables through PyHealth using a custom dataset config.
    2. Construct hourly LoS task samples with time-series, static features,
       and optional diagnosis features.
    3. Convert task samples into model-ready tensors.
    4. Train and validate the TPC model.
    5. Run architectural and feature-level ablations through command-line
       options.
...
Key Components:
    ...
    - build_diagnosis_vocab: Training-only diagnosis vocabulary construction
      using visit-level prevalence
    - SimpleLoSDataset: Wrapper that converts task samples into TPC-ready
      tensors
    - collate_fn: Variable-length batch padding for hourly sequences
    - evaluate: Validation loop with masked sequence metrics
    - main: Full dataset loading, training, validation, and ablation pipeline

Inputs:
    root: Path to the eICU dataset root directory. This may be supplied via
        ``--root`` or the ``EICU_ROOT`` environment variable.
    Task samples:
...
Outputs:
    ...
    - Ablation summary line reporting validation loss, MAE, and RMSE

Implementation Notes:
    - This script supports the ablation and example requirements of the
      project rubric by exposing configurable architectural and feature
      variants through CLI flags.
    - Supported ablations include:
        * full / temporal-only / pointwise-only model variants
        * shared vs feature-wise temporal convolutions
        * skip connections on/off
        * diagnosis inclusion/exclusion
        * categorical static inclusion/exclusion
        * decay channel ablations
        * MSLE vs MSE loss
    - The data pipeline is designed to work with synthetic, dev, or real
      eICU data, depending on the provided dataset root and CLI options.
    - Optional debug prediction printing may be enabled with ``--debug_examples``.

Example:
    >>> EICU_ROOT=/path/to/eicu_dataset PYTHONPATH=. \\
