...
class SyntheticSleepDataGenerator:
    ...
    def __init__(self, hours: int = 8, epoch_seconds: int = 30, seed: int = 42):
        self.hours = hours
    ...
    def _generate_patient_data(self, patient_id: str) -> dict:
        ...
        sleep_onset, final_wake, awakening_points, awakening_durations = (
            self._create_sleep_structure(has_apnea)
        )
    ...
    def _create_sleep_structure(self, has_apnea: int) -> tuple:
        ...
        if sleep_period > num_awakenings:
            awakening_points = sorted(
                random.sample(
                    range(sleep_onset, self.total_epochs - final_wake), num_awakenings
                )
            )
    ...
    def _generate_visits(
        ...
        sleep_onset: int,
        final_wake: int,
        awakening_points: list,
        awakening_durations: list,
        has_apnea: int,
    ) -> list:
        ...
        mislabel_indices = set(
            random.sample(range(self.total_epochs), int(0.05 * self.total_epochs))
        )

        # Generate time-series data for each epoch
        for t in range(self.total_epochs):
            # Determine ground truth sleep state
            true_state = self._determine_sleep_state(
                t, sleep_onset, final_wake, awakening_points, awakening_durations
            )

            # Handle transitions between states
            transition_factor = self._handle_state_transition(
                true_state, last_state, transition_momentum
            )
            ...
            # Apply label errors
            stage = (
                self.sleep_states[1 - true_state]
                if t in mislabel_indices
                else self.sleep_states[true_state]
            )
    ...
    def _handle_state_transition(
        self, true_state: int, last_state: int, momentum: int
    ) -> float:
    ...
    def _generate_signals(
        ...
        if true_state == 0:  # Awake
            ...
            eda_target = (
                baselines["eda"] + baselines["eda_diff"] + np.random.normal(0, 0.1)
            )
            temp_target = baselines["temp"] + np.random.normal(0, 0.1)
            acc_target = (
                baselines["acc"] + baselines["acc_diff"] + np.random.exponential(0.02)
            )
        else:  # Asleep
        ...
        if transition_factor > 0:
            hr_target = (
                hr_target * (1 - transition_factor) + history["hr"] * transition_factor
            )
            eda_target = (
                eda_target * (1 - transition_factor)
                + history["eda"] * transition_factor
            )
            temp_target = (
                temp_target * (1 - transition_factor)
                + history["temp"] * transition_factor
            )
            acc_target = (
                acc_target * (1 - transition_factor)
                + history["acc"] * transition_factor
            )
