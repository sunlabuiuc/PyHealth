"""
Example of using GAMENet for drug recommendation on MIMIC-IV.
...
4. Training a GAMENet model for multi-label classification
5. Evaluating with PR-AUC, F1, and Jaccard metrics

Model: GAMENet (Graph Augmented Memory Networks)
    Paper: Junyuan Shang et al. GAMENet: Graph Augmented MEmory Networks for
    Recommending Medication Combination. AAAI 2019.

    - Uses graph neural networks to model drug-drug interactions (DDI)
    - Incorporates memory networks to capture patient history
    - Jointly optimizes recommendation accuracy and DDI rate reduction

    IMPORTANT CONSTRAINTS:
    - Only accepts ATC level 3 medication codes
    - Requires 'conditions' and 'procedures' as input features
    - Requires 'drugs' as the output/label
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GAMENet for Drug Recommendation on MIMIC-IV"
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main():
    ...
    # codes
    # which is required by GAMENet for DDI graph modeling
    ...
    # STEP 4: Initialize GAMENet model
    # GAMENet requires 'conditions' and 'procedures' as input features
    # and 'drugs' as the output label (ATC level 3 codes)
    print("\nInitializing GAMENet model...")
    model = GAMENet(
