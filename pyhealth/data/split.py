...
def split_by_pat(dataset, ratios, batch_size=64):
    ...
    val_index_group = index_group[int(N_pat * ratios[0]):int(N_pat * (ratios[0] + ratios[1]))]
    ...
    return data_loader(train_dataset, batch_size, True), data_loader(val_dataset, batch_size, False), data_loader(test_dataset, batch_size, False)
