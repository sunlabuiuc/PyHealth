...
class ShapExplainer:
    ...
    def _compute_classic_shapley(
        ...
        # Determine batch size and initialize shap_values as (batch, n_features)
        if input_emb[key].dim() >= 2:
            batch_size = input_emb[key].shape[0]
        else:
            batch_size = 1

        shap_values = torch.zeros((batch_size, n_features), device=device)
        ...
        for i in range(n_features):
            ...
            for size in range(n_players):
                ...
                for coalition in itertools.combinations(other_features, size):
                    ...
                    # Set coalition features (handle sequence embeddings)
                    for j in coalition:
                        if input_emb[key].dim() == 3:
                    ...
                    marginal = pred_with_i - pred_without_i  # shape: (batch,)
                    weight = (
            ...
            # Average marginal contributions across coalitions -> per-sample
            # stack -> (n_coalitions, batch) -> mean over 0 -> (batch,)
            stacked = torch.stack(marginal_contributions, dim=0)
            mean_marginal = stacked.mean(dim=0)
            shap_values[:, i] = mean_marginal
    ...
    def _compute_kernel_shap_matrix(
        ...
        for _ in range(n_coalitions):
            ...
            for i, use_input in enumerate(coalition):
                if use_input:
                    # handle sequence embeddings (batch, seq_len, emb) vs (batch, n)
                    if input_emb[key].dim() == 3:
            ...
            if self.use_embeddings:
                ...
                # Forward pass (skip loss computation. SHAP doesn't need loss. It only needs predictions)
                with torch.no_grad():
                    label_stub = torch.zeros((mixed.shape[0], 1), device=self.model.device)   #temp
                ...
                if isinstance(model_output, dict) and "logit" in model_output:
                    ...
                    # Fallback: assume model_output is tensor
                    output = {"logit": model_output}


            else:
                # When calling model in non-embedding mode, ensure all feature
                # keys are present in kwargs. Use background values for other
                # features so the model receives full input batches of shape
                # (n_background, ...).
                model_inputs = {}
                for fk in self.model.feature_keys:
                    if fk == key:
                        ...
                        # Prefer background if provided, otherwise fall back to input_emb
                        if fk in background_emb:
                            model_inputs[fk] = background_emb[fk].clone()
                        elif fk in input_emb:
                            model_inputs[fk] = input_emb[fk].clone()
                        else:
                            # As a last resort, create zeros with batch dim equal to mixed
                            model_inputs[fk] = torch.zeros_like(mixed)

                # Provide a label stub matching batch size to avoid KeyError in
                # model.forward which may expect label for loss computation.
            ...
            # Get target class prediction (per-sample)
            if target_class_idx is None:
                pred = torch.max(logits, dim=-1)[0]  # shape: (batch,)
            else:
                pred = logits[..., target_class_idx]

            coalition_vectors.append(coalition.float().to(input_emb[key].device))
            # average predictions across background samples to obtain a scalar per coalition
            coalition_preds.append(pred.detach().mean())
        ...
        Y = torch.stack(coalition_preds, dim=0)    # (n_coalitions,)
        W = torch.stack(coalition_weights, dim=0)  # (n_coalitions,)

        # Weighted least squares via normal equations per sample
        # A = X^T W X, B = X^T W y -> solve A phi = B
        device = input_emb[key].device
        W_mat = torch.diag(W).to(device)
        XtW = X.t().to(device) @ W_mat
        A = XtW @ X.to(device)  # (n_features, n_features)
        # regularize
        A = A + 1e-6 * torch.eye(A.size(0), device=device)

        # Solve for single phi vector (we averaged over background earlier)
        B = XtW @ Y.to(device)
        phi = torch.linalg.solve(A, B)  # (n_features,)

        # Return as (1, n_features) to align with single-sample attribution
