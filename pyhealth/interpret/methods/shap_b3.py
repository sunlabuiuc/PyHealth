            ...
            for size in range(n_players):
                ...
                for coalition in itertools.combinations(other_features, size):
                    ...
                    if target_class_idx is None:
                        ...
                        # If model outputs multi-class logits, index directly.
                        if logits_without_i.dim() > 1 and logits_without_i.shape[-1] > 1:
                            pred_without_i = logits_without_i[..., target_class_idx]
                            pred_with_i = logits_with_i[..., target_class_idx]
                        else:
                            # Binary/single-logit output: interpret logits as score for class 1.
                            # Use sigmoid to get probabilities; for class 1 return sigmoid(logit),
                            # for class 0 return 1 - sigmoid(logit).
                            sig_without = torch.sigmoid(logits_without_i.squeeze(-1))
                            sig_with = torch.sigmoid(logits_with_i.squeeze(-1))
                            if target_class_idx == 1:
                                pred_without_i = sig_without
                                pred_with_i = sig_with
                            else:
                                pred_without_i = 1.0 - sig_without
                                pred_with_i = 1.0 - sig_with
    ...
    def _compute_deep_shap(
        ...
        if target_class_idx is None:
            ...
            if logits.dim() > 1 and logits.shape[-1] > 1:
                ...
                baseline_sig = torch.sigmoid(baseline_logits.squeeze(-1))
                if target_class_idx == 1:
                    pred = sig
                    baseline_pred = baseline_sig
                else:
                    pred = 1.0 - sig
                    baseline_pred = 1.0 - baseline_sig
            ...
            for b_idx in range(batch_size):
                ...
                for i, use_input in enumerate(coalition):
                    if use_input:
                        ...
                        if input_emb[key].dim() == 3:
                            # input_emb[key] shape: (batch, seq_len, emb)
