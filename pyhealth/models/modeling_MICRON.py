...
class MICRON(pl.LightningModule):
    def __init__(self, dataset, emb_dim=64):
        ...
        self.prescription_net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(emb_dim, self.output_len)
        )
    ...
    def embedding(self, conditions, procedures):
        ...
        health_representation = torch.cat([conditions_emb, procedures_emb], dim=1) # (#visit, emb_dim*2)
    ...
    def forward(self, conditions, procedures):
        ...
        health_representation_cur = torch.cat([conditions_emb_cur, procedures_emb_cur], dim=1) # (#visit-1, emb_dim*2)
        health_representation_last = torch.cat([conditions_emb_last, procedures_emb_last], dim=1) # (#visit-1, emb_dim*2)
        ...
        # reconstructon loss
        rec_loss = 1 / self.output_len * torch.sum(torch.pow((torch.sigmoid(drug_rep_cur) - torch.sigmoid(drug_rep_last + drug_residual_rep)), 2))
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        loss_bce = 0.75 * F.binary_cross_entropy_with_logits(result, drugs_multihot[1:]) + \
                (1 - 0.75) * F.binary_cross_entropy_with_logits(result_last, drugs_multihot[:-1])
