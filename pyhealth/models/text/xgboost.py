...
from pyhealth.data.data_reader.text import ml_reader
...
class XGBoost:
    ...
    def _build_model(self):
        ...
        if self.task_type == 'binaryclass':
            self.predictor = XGBClassifier(**_config, objective='binary:logistic', eval_metric="logloss")
        elif self.task_type == 'multiclass':
            self.predictor = XGBClassifier(**_config)
        elif self.task_type == 'multilabel':
            xgb_estimator = XGBClassifier(**_config, objective='binary:logistic', eval_metric="logloss")
    ...
    def fit(self, data_dict, X = None, y = None, assign_task_type = None):
        ...
        if data_dict != None:
            ...
            data = ml_reader.DatasetReader(data_dict, device = self.device, task_type = self.task_type).get_data()
            _X = np.array(data['X'])
            _y = np.array(data['Y'])
