...
class callPredictor(nn.Module):
    ...
    def __init__(self, 
        ...
        self.lweight = torch.nn.Parameter(torch.FloatTensor(label_size, fc_in_d, 1))
        self.output_func = nn.Linear(fc_in_d, 1)
    ...
    def forward(self, input_data):
        ...
        for conv_layer in self.conv_layers:
            x = conv_layer(x)
        feat_x = x.permute(0,2,1).reshape(n_case * n_visit, -1)
        hat_y_list = []
        for i in range(self.label_size):
            weights = torch.matmul(feat_x, self.lweight[i, :, :]).reshape(n_case, n_visit)
            weights = torch.exp(weights) * input_data['M']
            weights = weights/torch.sum(weights, -1).reshape(n_case, 1)
            ensemble_feat = torch.sum(torch.unsqueeze(weights, 1) * x, -1)
            cur_label = torch.sigmoid(self.output_func(ensemble_feat))
            hat_y_list.append(cur_label)
        hat_y = torch.cat(hat_y_list, -1)
        return _, hat_y
...
class callPredictor(nn.Module):
    def __init__(self, 
        ...
        self.lweight_set = nn.ParameterList([])
        self.foutput_set = nn.ModuleList([])
        
        self.n_level = n_level
        
        n_feat = layer_input_sizes[-1]
        for i in range(n_level):
            self.lweight_set.append(torch.nn.Parameter(torch.FloatTensor(label_size, n_feat, 1)))
            self.foutput_set.append(nn.Linear(n_feat, 1))
            n_feat += self.label_size
    ...
    def forward(self, input_data):
        ...
        n_case, n_visit, n_feat = X.shape
        _data = X
        for temp_rnn_model in self.rnn_models:
            _data, _ = temp_rnn_model(_data)
            _data = _data * M.unsqueeze(-1)
            
        x = _data
        for i in range(self.n_level):
            feat_x = x.reshape(n_case * n_visit, -1)
            hat_y_list = []
            for j in range(self.label_size):
                weights = torch.matmul(feat_x, self.lweight_set[i][i, :, :]).reshape(n_case, n_visit)
                weights = torch.exp(weights) * input_data['M']
                weights = weights/torch.sum(weights, -1).reshape(n_case, 1)
                ensemble_feat = torch.sum(torch.unsqueeze(weights, -1) * x, 1)
                cur_label = torch.sigmoid(self.foutput_set[i](ensemble_feat))
                hat_y_list.append(cur_label)
            hat_y = torch.cat(hat_y_list, -1)
            hat_y_feat = hat_y.unsqueeze(1).repeat(1,n_visit,1)
            x = torch.cat([x, hat_y_feat], -1)
            
        return _, hat_y

class JointLAAT(BaseControler):
    ...
    def __init__(self, 
        ...
        super(JointLAAT, self).__init__(expmodel_id)
