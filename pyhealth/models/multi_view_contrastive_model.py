...
class MultiViewContrastiveModel(BaseModel):
    ...
    def __init__(self, dataset, training_stage="pretrain", num_classes=3, **kwargs):
        ...
        self.encoder_t = nn.TransformerEncoder(encoder_layer, num_layers=3)
        self.encoder_d = nn.TransformerEncoder(encoder_layer, num_layers=3)
        self.encoder_f = nn.TransformerEncoder(encoder_layer, num_layers=3)
    ...
    def forward(self, **kwargs) -> dict[str, torch.Tensor]:
        ...
        temporal_tensor = self.proj_t(temporal_tensor)  # [N, L, hidden_dim]
        derivative_tensor = self.proj_d(derivative_tensor)  # [N, L, hidden_dim]
        frequency_tensor = self.proj_f(frequency_tensor)  # [N, L, hidden_dim]

        h_t = self.encoder_t(temporal_tensor)  # [N, L, hidden_dim]
        h_d = self.encoder_d(derivative_tensor)  # [N, L, hidden_dim]
        h_f = self.encoder_f(frequency_tensor)  # [N, L, hidden_dim]

        batch_size, seq_length, _ = h_t.shape
        H = torch.stack([h_t, h_d, h_f], dim=2)  # [N, 2, L, hidden_dim]
        H_flat = H.view(-1, 3, self.hidden_dim)

        MHA_out, _ = self.fusion_mha(H_flat, H_flat, H_flat)  # [N*L, 3, hidden_dim]
        MHA_out = self.fusion_layer_norm(MHA_out + H_flat)  # Residual connection

        # Split the MHA output back into individual views
        MHA_out = MHA_out.view(batch_size, seq_length, 3, self.hidden_dim)
        h_t_star, h_d_star, h_f_star = MHA_out[:, :, 0, :], MHA_out[:, :, 1, :], MHA_out[:, :, 2, :]
        z_t = self.F_t(h_t_star).mean(dim=1)  # [N, L, hidden_dim] -> [N, hidden_dim]
        z_d = self.F_d(h_d_star).mean(dim=1)  # [N, L, hidden_dim] -> [N, hidden_dim]
        z_f = self.F_f(h_f_star).mean(dim=1)  # [N, L, hidden_dim] -> [N, hidden_dim]

        # --- Stage Routing ---
        if self.training_stage == "pretrain":
            z_t_aug, z_d_aug, z_f_aug = self.augment(z_t), self.augment(z_d), self.augment(z_f)
