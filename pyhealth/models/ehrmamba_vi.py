...
class EHRMamba(BaseModel):
    ...
    def __init__(
        ...
        # PROJECT ADDITION (not in PyHealth's ehrmamba.py): conditional embedding
        # selection.  PyHealth's version unconditionally uses EmbeddingModel.
    ...
    def forward(self, **kwargs: Any) -> Dict[str, torch.Tensor]:
        ...
        # PROJECT ADDITION (not in PyHealth's ehrmamba.py): passes auxiliary
        # clinical metadata to EHRMambaEmbeddingAdapter before the embedding
        # forward call.  set_aux_inputs() caches token_type_ids, time_stamps,
        # ages, visit_orders, and visit_segments so EHRMambaEmbedding can apply
        # the full §2.2 / Eq. 1 embedding.  The guard on "token_type_ids" ensures
        # this is a no-op when using PyHealth's standard EmbeddingModel, which
        # does not produce these fields and does not have set_aux_inputs().
