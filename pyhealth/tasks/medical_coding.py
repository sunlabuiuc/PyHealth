...
class MIMIC3ICD9Coding(BaseTask):
    ...
    def __call__(self, patient: Patient) -> List[Dict]:
        ...
        for admission in admissions:
            ...
            # Get notes for this specific admission
            noteevents = patient.get_events(
                event_type="noteevents",
                filters=[("hadm_id", "==", hadm_id)],  # Filter by hadm_id
            )

            # Get diagnoses and procedures for this specific admission
            ...
            # Combine text from all notes for this admission
            text = ""
            ...
            icd_codes = list(set(diagnoses_codes + procedures_codes))

            # Only add sample if there are both notes and codes
            if text and icd_codes:
                samples.append(
                    {
                ...
                logger.warning(
                    f"Patient {patient.patient_id} has no clinical notes for admission {hadm_id}"
                )
            elif len(icd_codes) < 1:
                logger.warning(
                    f"Patient {patient.patient_id} has no ICD codes for admission {hadm_id}"
                )
