...
class NotesLabsMIMIC4(BaseMultimodalMIMIC4Task):
    ...
    def __call__(self, patient: Any) -> List[Dict[str, Any]]:
        ...
        for admission in admissions_to_process:
            ...
            # exam time, so they're bounded to the same window as labs/vitals
...
class NotesLabsCXRMIMIC4(BaseMultimodalMIMIC4Task):
    ...
    ICD codes and vitals are excluded by default but can be re-enabled for
    ablation experiments, same as ``NotesLabsMIMIC4``.
    ...
    Args:
        window_hours: Hours from admission for lab/vital/CXR collection.
...
class CXRMIMIC4(BaseMultimodalMIMIC4Task):
    ...
    no notes, no ICD codes, no labs/vitals — isolating the chest X-ray
