...
def sleep_staging_isruc_fn(record, epoch_seconds=10, label_id=1):
    ...
    def _find_channels(potential_channels):
        ...
        for c in potential_channels:
            ...
            new_c = c.replace("-M2", "").replace("-A2", "").replace("-M1", "").replace("-A1", "")
            if new_c in _channels:
                assert new_c not in keep, f"Unrecognized channels: {potential_channels}"
                keep[new_c] = c
        assert len(keep) == len(_channels), f"Unrecognized channels: {potential_channels}"
        return {v:k for k,v in keep.items()}

    record = record[0]
    save_path = os.path.join(record['save_to_path'], f'{epoch_seconds}-{label_id}', record['subject_id'])
    if not os.path.isdir(save_path):
        os.makedirs(save_path)
    data = mne.io.read_raw_edf(os.path.join(record['load_from_path'], record['signal_file'])).to_data_frame()
    data = data.rename(columns=_find_channels(data.columns)).reindex(columns=_channels).values
    ann = pd.read_csv(os.path.join(record['load_from_path'], record[f'label{label_id}_file']), header=None)[0]
