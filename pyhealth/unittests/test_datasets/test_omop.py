...
class TestOMOPDataset(unittest.TestCase):
    ...
    def test_patient(self):
        ...
        expected_available_tables = ['drug_exposure', 'procedure_occurrence', 'measurement']
        ...
        expected_available_tables_for_visit = ['procedure_occurrence', 'drug_exposure', 'measurement']
        ...
        Expected_data = collections.namedtuple('Expected_data_attributes', ['code', 'timestamp', 'vocabulary', ])
        
        expected_event_list_dict = [
            ('procedure_occurrence', 4, 0, Expected_data('2005280', datetime.datetime(2008, 8, 8, 0, 0), 'PROCEDURE_CONCEPT_ID')),
            ('drug_exposure', 1, 0, Expected_data('2213483', datetime.datetime(2008, 8, 8, 0, 0), 'DRUG_CONCEPT_ID')),
            ('measurement', 1, 0, Expected_data('2212095', None, 'MEASUREMENT_CONCEPT_ID'))
        ]
        ...
        self.assertEqual(sorted(expected_available_tables), sorted(actual_patient.available_tables))
        ...
        self.assertEqual(expected_visit_id, actual_patient.index_to_visit_id[selected_visit_index])
        
        actual_visit_id = actual_patient.index_to_visit_id[selected_visit_index]
        
        actual_visit = actual_patient.visits[actual_visit_id]
        
        self.assertEqual(expected_available_tables_for_visit, actual_visit.available_tables)
        ...
        self.assertEqual(expected_visit_len_event_list_dict, len(actual_visit.event_list_dict))
        
        
        
        for expected_event_type, expected_event_list_len, selected_event_index, expected_event_data in expected_event_list_dict:
