...
class eICUDataset(unittest.TestCase):
    ...
    ROOT = "~/Code/PyHealth_sunlab/PyHealth/datasets/eicu-collaborative-research-database-demo-2.0.1"
    ...
    def test_patient(self):
        ...
        # expect:
        # patient data
        expected_birth_datetime = datetime.datetime(1938, 2, 18, 0, 0, 0)
        ...
        expected_discharge_time = datetime.datetime(2014, 2, 21, 0, 45)
        expected_encounter_time = datetime.datetime(2014, 2, 18, 2, 59)
        ...
        expected_event_data = {
            'diagnosis': {
                'length': 8,
                'events': [
                    (0, Event(code='567.9', timestamp=datetime.datetime(2014, 2, 18, 3, 36, 0), vocabulary='ICD9CM')), 
                    (1, Event(code='K65.0', timestamp=datetime.datetime(2014, 2, 18, 3, 36, 0), vocabulary='ICD10CM'))
                ]
            },
            'medication': {
                'length': 38,
                'events': [
                    (0, Event(code='MORPHINE INJ', timestamp=datetime.datetime(2014, 2, 17, 21, 9, 0), vocabulary='eICU_DRUGNAME')),
                    (5, Event(code='CIPROFLOXACIN IN D5W 400 MG/200ML IV SOLN', timestamp=datetime.datetime(2014, 2, 17, 22, 43, 0), vocabulary='eICU_DRUGNAME'))
                ]
            },
            'lab': {
                'length': 251,
                'events': [ 
                    (0, Event(code='sodium', timestamp=datetime.datetime(2014, 2, 17, 21, 4, 0), vocabulary='eICU_LABNAME')),
                    (2, Event(code='BUN', timestamp=datetime.datetime(2014, 2, 17, 21, 4, 0), vocabulary='eICU_LABNAME'))
                ]
            },
            'physicalExam': {
                'length': 22,
                'events': [
                    (0, Event(code='notes/Progress Notes/Physical Exam/Physical Exam/Neurologic/GCS/Score/scored', timestamp=datetime.datetime(2014, 2, 18, 3, 5, 0), vocabulary='eICU_PHYSICALEXAMPATH')),
                    (1, Event(code='notes/Progress Notes/Physical Exam/Physical Exam Obtain Options/Performed - Structured', timestamp=datetime.datetime(2014, 2, 18, 3, 5, 0), vocabulary='eICU_PHYSICALEXAMPATH'))   
                ]
            }
        }
        ...
        self.assertEqual(expected_visit_hopital_id, actual_visit_attributes['hospital_id'])
        ...
        for event_key in expected_event_data:
            ...
            self.assertEqual(expected_event['length'], len(actual_event_array), f"incorrect num events for'{event_key}'")
