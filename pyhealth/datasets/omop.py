...
class OMOPBaseDataset(BaseDataset):
    ...
    def __init__(self, root, files=['conditions', 'procedures', 'prescriptions', 'labs']):
        ...
        if not os.path.exists(os.path.join(str(Path.home()), ".cache/pyhealth/omop.data")):
            ...
            dump_pickle(patients, os.path.join(str(Path.home()), ".cache/pyhealth/omop.data"))
        else:
            patients = load_pickle(os.path.join(str(Path.home()), ".cache/pyhealth/omop.data"))
    ...
    def parse_conditions(self, visits, patients):
        ...
        for visit_id, visit_info in tqdm(condition_df.groupby("visit_occurrence_id")):
            ...
            for code, time_datetime, time_date in visit_info[
                ["condition_concept_id", "condition_start_datetime", "condition_start_date"]].values:
    ...
    def parse_procedures(self, visits, patients):
        ...
        for visit_id, visit_info in tqdm(procedure_df.groupby("visit_occurrence_id")):
            ...
            cur_procedure = []
            for code, time_datetime, time_date in visit_info[
                ["procedure_concept_id", "procedure_start_datetime", "procedure_start_date"]].values:
                if time_date == time_date:
    ...
    def parse_prescriptions(self, visits, patients):
        ...
        for visit_id, visit_info in tqdm(drugs_df.groupby("visit_occurrence_id")):
            ...
            cur_drugs = []
            for code, time_datetime, time_date in visit_info[
                ["drug_concept_id", "drug_exposure_start_datetime", "drug_exposure_start_date"]].values:
                if time_date == time_date:
    ...
    def parse_labs(self, visits, patients):
        ...
        for visit_id, visit_info in tqdm(labs_df.groupby("visit_occurrence_id")):
            ...
            cur_labs = []
            for code, time_datetime, time_date in visit_info[
                ["measurement_concept_id", "measurement_datetime", "measurement_date"]].values:
                if time_date == time_date:
