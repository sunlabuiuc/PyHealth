...
class OMOPBaseDataset(BaseDataset):
    ...
    def __init__(self, root, files=['conditions', 'procedures', 'prescriptions', 'labs']):
        """
        INPUT
    ...
    def parse_visit_occurrence(self):
        ...
        visit_occurrence_df = pd.read_csv(os.path.join(self.root, "visit_occurrence.csv"), sep="\t",
                        dtype={'person_id': str, 'visit_occurrence_id': str})
        death_df = pd.read_csv(os.path.join(self.root, "death_filled.csv"), sep="\t",
                        dtype={'person_id': str})
        visit_occurrence_df = pd.merge(visit_occurrence_df, death_df, on='person_id', how='left')
        visits = {}
    ...
    def parse_conditions(self, visits, patients):
        ...
        condition_df = pd.read_csv(os.path.join(self.root, "condition_occurrence.csv"), sep="\t", \
        ...
        for visit_id, visit_info in tqdm(condition_df.groupby("visit_occurrence_id")):
            ...
            cur_condition = []
            for code, time_datetime, time_date in visit_info[["condition_concept_id", "condition_start_datetime", "condition_start_date"]].values:
            ...
            if len(cur_condition) == 0: continue
    ...
    def parse_procedures(self, visits, patients):
        ...
        procedure_df = pd.read_csv(os.path.join(self.root, "procedure_occurrence.csv"), sep="\t", \
            dtype={'person_id': str, "visit_procedure_id": str, "procedure_concept_id": str})
        for visit_id, visit_info in tqdm(procedure_df.groupby("visit_occurrence_id")):
            if visit_id not in visits: continue

            # load procedure with time info
            cur_procedure = []
            for code, time_datetime, time_date in visit_info[["procedure_concept_id", "procedure_start_datetime", "procedure_start_date"]].values:
                if time_date == time_date:
                    cur_procedure += self.process_nested_code(code, time_date)
                else:
                    cur_procedure += self.process_nested_code(code, time_datetime)
                
            if len(cur_procedure) == 0: continue
    ...
    def parse_prescriptions(self, visits, patients):
        """ func to parse condition table """
        drugs_df = pd.read_csv(os.path.join(self.root, "drug_exposure.csv"), sep="\t", \
        ...
        for visit_id, visit_info in tqdm(drugs_df.groupby("visit_occurrence_id")):
            ...
            cur_drugs = []
            for code, time_datetime, time_date in visit_info[["drug_concept_id", "drug_exposure_start_datetime", "drug_exposure_start_date"]].values:
                if time_date == time_date:
                    cur_drugs += self.process_nested_code(code, time_date)
                else:
                    cur_drugs += self.process_nested_code(code, time_datetime)
                
            if len(cur_drugs) == 0: continue
    ...
    def parse_labs(self, visits, patients):
        """ func to parse condition table """
        labs_df = pd.read_csv(os.path.join(self.root, "measurement_filled.csv"), sep="\t", \
            dtype={'person_id': str, "visit_occurrence_id": str, "measurement_concept_id": str})
        for visit_id, visit_info in tqdm(labs_df.groupby("visit_occurrence_id")):
            if visit_id not in visits: continue

            # load labs with time info
            cur_labs = []
            for code, time_datetime, time_date in visit_info[["measurement_concept_id", "measurement_datetime", "measurement_date"]].values:
                if time_date == time_date:
                    cur_labs += self.process_nested_code(code, time_date)
                else:
                    cur_labs += self.process_nested_code(code, time_datetime)
                
            if len(cur_labs) == 0: continue
