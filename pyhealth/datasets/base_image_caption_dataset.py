...
dataset.patients:
    - key: patient id
    - value: a dict of image paths, captions, and other information
"""
...
class BaseImageCaptionDataset(ABC):
    ...
    def __init__(
        ...
        self.dev = dev
        if dev:
            logger.warning("WARNING: dev has no effect for image caption generation datasets.")
        # TODO: refresh_cache seems unnecessary for image and signal?
        self.refresh_cache = refresh_cache
        if refresh_cache:
            logger.warning("WARNING: refresh_cache has no effect for image caption generation datasets.")

        self.metadata = pd.read_json(os.path.join(root, "metadata.jsonl"), lines=True)
    ...
    def set_task(
        ...
        Note:
            In `task_fn`, a patient may have one or multiple images associated
                to a caption, for e.g. a +patient can have single report associated
                to multiple xrays from diffrent views that may be combined to have 
                a single sample ({'patient_id':1, 'frontal_image':'img1',
                'lateral_image': 'img2','report':'text})
