...
from .sample_dataset import SampleDataset, SampleSubset
...
def split_by_patient(
    ...
    train_dataset = SampleSubset(dataset, train_index)
    val_dataset = SampleSubset(dataset, val_index)
    test_dataset = SampleSubset(dataset, test_index)
    return train_dataset, val_dataset, test_dataset
...
def split_by_sample(
    ...
    test_index = index[int(len(dataset) * (ratios[0] + ratios[1])) :]
    train_dataset = SampleSubset(dataset, train_index.tolist())
    val_dataset = SampleSubset(dataset, val_index.tolist())
    test_dataset = SampleSubset(dataset, test_index.tolist())
...
def split_by_visit_conformal(
    ...
    Returns:
        train_dataset, val_dataset, cal_dataset, test_dataset: four subsets
            of the dataset of type `SampleSubset`.
    """
    if seed is not None:
        np.random.seed(seed)
...
def split_by_sample_conformal(
    ...
    Returns:
        train_dataset, val_dataset, cal_dataset, test_dataset: four subsets
            of the dataset of type `SampleSubset`, or four tensors
