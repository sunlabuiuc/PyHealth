...
class eICUDataset(BaseEHRDataset):
    ...
    We further support the following tables:
        ...
        - diagnosisString: contains the diagnosis information (eICU_DIAGNOSISSTRING)
            for patients
    ...
    Examples:
        ...
        ...         tables=["diagnosis", "medication", "lab", "treatment", "physicalExam", "admissionDx", "diagnosisString"],
        ...     )
        >>> dataset.stat()
        >>> dataset.info()
    """
    ...
    def parse_diagnosisstring(self, patients: Dict[str, Patient]) -> Dict[str, Patient]:
        """Helper function which parses diagnosis string table, which is the diagnosisString
        column under the diagnosis table.
        ...
        df = pd.read_csv(
            os.path.join(self.root, f"{table}.csv"),
            dtype={"patientunitstayid": str, "diagnosisstring": str},
        )
        # drop rows with missing values
        df = df.dropna(subset=["patientunitstayid", "diagnosisstring"])
        # sort by diagnosisoffset
        df = df.sort_values(["patientunitstayid", "diagnosisoffset"], ascending=True)
        # add the patient id info
        df["patient_id"] = df["patientunitstayid"].apply(
        ...
        # parallel unit of diagnosisString (per visit)
        def diagnosisString_unit(v_info):
            v_id = v_info["patientunitstayid"].values[0]
            patient_id = v_info["patient_id"].values[0]
            v_encounter_time = v_info["v_encounter_time"].values[0]
            if patient_id is None:
            ...
            events = []
            for offset, code in zip(
                    v_info["diagnosisoffset"], v_info["diagnosisstring"]
            ):
                # compute the absolute timestamp
                timestamp = v_encounter_time + pd.Timedelta(minutes=offset)
                event = Event(
                    code=code,
                    table="diagnosisString",
                    vocabulary="eICU_DIAGNOSISSTRING",
                    visit_id=v_id,
                    patient_id=patient_id,
                    timestamp=timestamp,
                )
        ...
        # parallel apply
        group_df = group_df.parallel_apply(lambda x: diagnosisString_unit(x))
