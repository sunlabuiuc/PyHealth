...
AMINO_ACID_DICT = {'A':1,'R':2,'N':3,'D':4,'C':5,'Q':6,'E':7,'G':8,'H':9,'I':10,'L':11,'K':12,'M':13,'F':14,
            'P':15,'O':16,'S':17,'U':18,'T':19,'W':20,'Y':21,'V':22}

SPECIES= {
    
'eukaryotes': ["Anopheles gambiae", "Arabidopsis thaliana", "Aspergillus niger", "Bos taurus", "Caenorhabditis elegans", "Candida albicans", 
"Chlamydomonas reinhardtii", "Dictyostelium_discoideum", "Drosophila melanogaster", "Danio rerio", "Gallus gallus", "Homo sapiens", 
"Mus musculus", "Plasmodium falciparum", "Rattus norvegicus", "Schizosaccharomyces pombe", "Saccharomyces cerevisiae", "Trypanosoma_brucei"],
                
'prokaryotes': ["Bacillus anthracis", "Bacillus_subtilis_168", "Bacteroides thetaiotaomicron VPI-5482", "Escherichia coli K-12", 
"Halobacterium salinarum R1", "Helicobacter_pylori_26695", "Lactococcus lactis Il1403", "Legionella_pneumophila_Philadelphia_1", 
"Listeria_monocytogenes_EGD_e", "Mycobacterium tuberculosis", "Neisseria_meningitidis_MC58", "Pseudomonas_aeruginosa_PAO1", 
"Salmonella_enterica_serovar_Typhimurium_14028S", "Shewanella_oneidensis_MR_1", "Shigella dysenteriae", "Staphylococcus_aureus_JH1", 
"Streptococcus pyogenes", "Yersinia_pestis_CO92"],
     
'viruses': ["Human adenovirus A", "Human adenovirus C", "Human herpesvirus 1", "Human herpesvirus 4", "Human herpesvirus 5", 
"Human immunodeficiency virus 1", "Influenza A virus H5N1", "Macacine herpesvirus 3", "Moloney murine leukemia virus", 
"Monkeypox_virus_Zaire_96_I_16", "Murid herpesvirus 4", "Murine_type_C_retrovirus", "Pandoravirus salinus", "Pithovirus_sibericum", 
"Saccharomyces_cerevisiae_virus_L_A", "Saccharomyces_cerevisiae_virus_L_BC", "Vaccinia_virus"],
     
'special': ["cRAP"]            
}

BASE_URL = "https://gpmdb.thegpm.org/thegpm-cgi/peptides_by_species.pl"
DATA_PATH = 'default_gpm_dataset'
...
class GPMDataset(Dataset):
  ...
  class GenerateData():
    def __init__(self, aa_map, max_seq_length, species, base_url, data_path, verbose, download_option):
      ...
      if self.download_option != GPMDataset.NONE:
        ...
        self.vprint(f'Mapping amino acid sequences of length {self.max_seq_length} with map of size {len(self.aa_map)}')
    ...
    def vprint(self, message, indent=0):
      ...
      if self.verbose: print(indent*'\t', message)
    ...
    def download_data(self):
      ...
      with requests.Session() as session:
        ...
        for spec in itertools.chain.from_iterable(self.species.values()):
          ...
          resp = None
          try:
            resp = session.get(f'{self.base_url}?species={spec}', headers=headers)
            if resp.status_code == 200:

              with open(f'{os.path.join(self.data_path, spec)}.tsv', 'w', encoding='utf-8') as specie_file:
                specie_file.write(resp.text)

          except (pd.errors.ParserError, requests.exceptions.ConnectionError, requests.exceptions.ChunkedEncodingError, requests.exceptions.ReadTimeout) as e:
  ...
  def __init__(self, base_url=BASE_URL, data_path=DATA_PATH, aa_map=AMINO_ACID_DICT, max_seq_length=MAX_SEQ_LENGTH, 
               species=SPECIES, verbose=False, dataset_name=None, download_option='remote'):
    'Initialization'
    self.gen = self.GenerateData(aa_map, max_seq_length, species, base_url, data_path, verbose, download_option)
    ...
    if download_option not in self.DOWNLOAD_OPTIONS:
      raise ValueError(f'Invalid download option. Must be one of {self.DOWNLOAD_OPTIONS}')
    self.download_option = download_option

    if dataset_name: self.dataset_name = dataset_name
    else: self.dataset_name = self.data_path
  ...
  def train_test_split(self, train_frac, shuffle=True, train_dataset_name=None, test_dataset_name=None):
    # split dataset into training and testing datasets, returning two GPMDatasets
    if not (train_frac > 0 and train_frac < 1): 
      raise ValueError(f'Failed to generate datasets. Training fraction {train_frac} must be > 0 and < 1.')

    if shuffle: data = self.data.sample(frac=1)
    else: data = self.data

    train_dataset_name = train_dataset_name if train_dataset_name else f'{self.dataset_name}_train'
    test_dataset_name = test_dataset_name if test_dataset_name else f'{self.dataset_name}_test'

    train_data, test_data = train_test_split(data, test_size=1-(train_frac))

    train_gpmdb = GPMDataset(base_url=self.base_url, data_path=self.data_path, aa_map=self.aa_map, 
                             max_seq_length=self.max_seq_length, species=self.species, verbose=self.verbose,
                             dataset_name=train_dataset_name, download_option=self.NONE)                             
    train_gpmdb.data = train_data

    test_gpmdb = GPMDataset(base_url=self.base_url, data_path=self.data_path, aa_map=self.aa_map, 
                             max_seq_length=self.max_seq_length, species=self.species, verbose=self.verbose,
                             dataset_name=test_dataset_name, download_option=self.NONE)
