...
dataset.patients:
    - key: patient id
    - value: a dict of features, labels, and other information
"""


class BaseImageDataset(ABC):
    """Abstract base Image dataset class.

    This abstract class defines a uniform interface for all image datasets.
    ...
    def __init__(
        ...
        self.dev = dev
        if dev:
            logger.warning("WARNING: dev has no effect for image datasets.")
        # TODO: refresh_cache seems unnecessary for image and signal?
        self.refresh_cache = refresh_cache
        if refresh_cache:
            logger.warning("WARNING: refresh_cache has no effect for image datasets.")
