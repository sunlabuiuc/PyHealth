"""Dataset loader for VQA-RAD style medical visual question answering data."""
...
class VQARADDataset(BaseDataset):
    """Base dataset wrapper for VQA-RAD style VQA data.

    This implementation supports flexible local layouts by auto-detecting
    annotation files and image folders, with optional split overrides and
    lightweight dedup hooks for leakage mitigation.
    """
    ...
    def __init__(
        ...
        if config_path is None:
            config_path = str(Path(__file__).parent / "configs" / "vqa_rad.yaml")

        metadata_path = root_path / "vqa_rad-metadata-pyhealth.csv"
        ...
        super().__init__(
            root=str(root_path),
            tables=["vqa_rad"],
            dataset_name=dataset_name or "VQARADDataset",
    ...
    def _detect_annotation_path(self, root: Path) -> Path:
        ...
        for candidate in candidates:
            ...
            if path.exists() and path.is_file():
                logger.info("Using VQA-RAD annotation file: %s", path)
                return path

        raise FileNotFoundError(
            "Could not auto-detect VQA-RAD annotations. Set annotation_path or "
    ...
    def prepare_metadata(self, root: Path) -> None:
        if self.clean_split_path is None and not self.enable_dedup:
            warnings.warn(
                "No clean_split_path and dedup disabled. Official VQA-RAD split "
                "can contain leakage; results may be inflated.",
                UserWarning,
                stacklevel=2,
            )
        ...
        for row_idx, record in enumerate(records):
            ...
            try:
                ...
                logger.warning("Skipping malformed VQA-RAD row %d: %s", row_idx, exc)
                continue
            rows.append(row)
        ...
        if not rows:
            raise ValueError("No valid VQA-RAD records found after preprocessing")

        self._validate_rows(rows)

        metadata = pd.DataFrame(rows)
        metadata_path = root / "vqa_rad-metadata-pyhealth.csv"
        metadata.to_csv(metadata_path, index=False)
        logger.info("Saved VQA-RAD metadata to %s", metadata_path)
