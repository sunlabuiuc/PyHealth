...
class TestCheck(unittest.TestCase):
    ...
    def test_check_model_dir(self):
        ...
        assert os.path.exists(
            os.path.join('./experiments_records', test_model_id))
        assert os.path.exists(
            os.path.join('./experiments_records', test_model_id, 'checkouts'))
        assert os.path.exists(
            os.path.join('./experiments_records', test_model_id, 'results'))
        os.rmdir(
            os.path.join('./experiments_records', test_model_id, 'checkouts'))
        os.rmdir(
            os.path.join('./experiments_records', test_model_id, 'results'))
