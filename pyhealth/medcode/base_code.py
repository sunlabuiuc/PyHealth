...
class BaseCode(ABC):
    ...
    def download_and_read_csv(filename: str, refresh_cache: bool = False):
        if (not os.path.exists(os.path.join(MODULE_CACHE_PATH, filename))) or refresh_cache:
            download(urljoin(BASE_URL, filename), os.path.join(MODULE_CACHE_PATH, filename))
    ...
    def get_ancestors(self, code):
        ...
        ancestors = sorted(ancestors, key=lambda x: nx.shortest_path_length(self.graph, x, code))
        return ancestors
    ...
    def load_mapping(self, target_vocabulary):
        ...
        if os.path.exists(pickle_filepath) and not self.refresh_cache:
            ...
            print(f"Saved {self.vocabulary}->{target_vocabulary} mapping to {pickle_filepath}")
