...
class KGEBaseModel(ABC, nn.Module):
    ...
    def data_process(self, sample_batch, mode):
        """ Data process function which converts the data batch into head, relation, tail

        Args:
            sample_batch: a batch of triples of shape (batch_size, )
        
        
        
        """
        batch_size, negative_sample_size = sample_batch.size(0), 1 if mode == 'clf' else sample_batch.size(1)

        head_part, relation_part, tail_part = torch.split(sample_batch, [1, 1, 1], dim=1) if mode == 'clf' else (sample_batch[:, 0:1], sample_batch[:, 1:2], sample_batch[:, 2:3])
    ...
    def forward(self, sample_batch, mode='clf'):
