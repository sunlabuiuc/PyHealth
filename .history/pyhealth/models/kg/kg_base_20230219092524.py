...
class KGEBaseModel(ABC, nn.Module):
    ...
    def forward(self, batch, mode='clf'):
        """ Forward function which calculates the score of a tirple batch

        Args:
            batch: a batch of triples of shape (batch_size, )
        
        
        
        """
        batch_size, negative_batch_size = batch.size(0), 1 if mode == 'clf' else batch.size(1)

        head_part, relation_part, tail_part = torch.split(batch, [1, 1, 1], dim=1) if mode == 'clf' else (batch[:, 0:1], batch[:, 1:2], batch[:, 2:3])

        head_index = (head_part.view(-1) if mode == 'head' else head_part[:, 0]) if mode != 'clf' else head_part[:, 0]
        tail_index = (tail_part.view(-1) if mode == 'tail' else tail_part[:, 0]) if mode != 'clf' else tail_part[:, 0]

        head = self.entity_embedding[head_index].unsqueeze(1)
        relation = self.relation_embedding[relation_part[:, 0]].unsqueeze(1)
        tail = self.entity_embedding[tail_index].view(batch_size, negative_batch_size, -1)
