...
class KGEBaseModel(ABC, nn.Module):
    ...
    def forward(self, sample, mode='classification'):
        ...
        """
        batch_size, negative_sample_size = sample.size(0), 1 if mode == 'classification' else sample.size(1)

        head_part, relation_part, tail_part = torch.split(sample, [1, 1, 1], dim=1) if mode == 'classification' else (sample[:, 0:1], sample[:, 1:2], sample[:, 2:3])

        head_index = (head_part.view(-1) if mode == 'head' else head_part[:, 0]) if mode != 'single' else head_part[:, 0]
        tail_index = (tail_part.view(-1) if mode == 'tail' else tail_part[:, 0]) if mode != 'single' else tail_part[:, 0]
