#!/usr/bin/env bash
# HTCondor executable — labs-only Transformer mortality run.
#
# Condor equivalent of scripts/will/sunlab/tmux_run_labs_only_rnn_variant.py:
# same task (labs), same defaults pattern as run_labs_only_rnn.sh but for the
# transformer model. GPU selection (nvidia-smi / CUDA_VISIBLE_DEVICES) is
# dropped since Condor assigns the GPU via request_gpus / cgroups.
#
# usage: run_labs_only_transformer.sh <seed>
# to remove logs: rm -rf logs/condor/*
# to remove cache: rm -rf /shared/eng/wp14/pyhealth_cache_labs/*
set -euo pipefail

SEED="${1:?usage: run_labs_only_transformer.sh <seed>}"
...
JOB_TAG="transformer_labs_s${SEED}_c${_CONDOR_CLUSTER_ID:-local}_p${_CONDOR_PROCNO:-0}"

echo "========================================================"
echo "  Labs-only Transformer run  |  ${JOB_TAG}"
